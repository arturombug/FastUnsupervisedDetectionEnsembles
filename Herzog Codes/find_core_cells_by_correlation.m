...
parfor s=1:nsur
    sur_ens_seq = shuffle(ens_raster,'time'); %remove by arturo
    %sur_ens_seq = logical(shuffle(ens_raster,'time')); %add by arturo
    sur_cel_cor(:,:,s)=1-pdist2(double(raster),double(sur_ens_seq),'correlation'); % correlation between cell and ensemble
end
